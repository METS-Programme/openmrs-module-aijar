...
/**
 * Exits patients, who have lost follow up, from the MCH program
 */
public class ExitMCHProgramASLostToFollowUpTask extends AbstractTask {

	public static final int LOST_TO_FOLLOWUP_CONCEPT_ID = 5240;
	
	private int lostToFollowUpDays = 315;
	...
	public void execute() {
		...
		for (PatientProgram patientProgram : service.getPatientPrograms(null, tbProgram, null, null, null, null, false)) {
			...
			if (lostToFollowUp(patientProgram.getPatient())) {
				...
				service.voidPatientProgram(patientProgram, "ExitMCHProgramTask Lost to followup");
			}
		}
	}
