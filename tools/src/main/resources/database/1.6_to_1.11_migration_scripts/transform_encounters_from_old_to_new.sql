...
-- ART SUMMARY
update openmrs_backup.encounter_type set encounter_type_id = 50 where encounter_type_id = (select encounter_type_id from openmrs.encounter_type where locate('art',name) > 0 and locate('summary',name) > 0);
update openmrs_backup.form set form_id = 50 where form_id = (select form_id from openmrs.form where locate('art',name) > 0 and locate('card',name) > 0 and locate('summary',name) > 0);
...
-- ART Encounter Page
update openmrs_backup.encounter_type set encounter_type_id = 60 where encounter_type_id = (select encounter_type_id from openmrs.encounter_type where locate('art',name) > 0 and locate('card',name) > 0 and locate('encounter',name) > 0 and locate('education',name) = 0);
update openmrs_backup.form set form_id = 60 where form_id = (select form_id from openmrs.form where locate('art',name) > 0 and locate('card',name) > 0 and locate('encounter',name) > 0);
...
update openmrs_backup.encounter_type set encounter_type_id = 70 where encounter_type_id = (select encounter_type_id from openmrs.encounter_type where locate('art',name) > 0 and locate('card',name) > 0 and locate('education',name) > 0);
update openmrs_backup.form set form_id = 70 where form_id = (SELECT form_id FROM openmrs.form WHERE locate('art', name) > 0 AND locate('card', name) > 0 AND locate('education', name) > 0);
