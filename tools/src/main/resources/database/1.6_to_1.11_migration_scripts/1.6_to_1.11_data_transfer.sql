...
CREATE DEFINER=`openmrs`@`localhost` PROCEDURE `transfer`()
  BEGIN
    ...
    read_loop: LOOP
      ...
      SELECT CONCAT('Processing ',t_name,' table');

      -- Get column names for table from the backup database

      SELECT CONCAT('Getting columns of ',t_name,' table from the backup database');
      ...
      SELECT CONCAT('Getting columns of ',t_name,' table from the new database');
      ...
      SELECT CONCAT('Getting primary keys for  ',t_name,' table from old database');
      ...
      SELECT CONCAT('Constructing where clause for table ',t_name,' to join the old database to the new database');
      ...
      SELECT CONCAT('Finding intersecting columns between the old and the new database for table  ',t_name);
      ...
      IF(inter_columns is not null AND inter_columns <> '') THEN
        ...
        SELECT CONCAT('Creating the data transfer sql statement from the old database to the new database for table  ',t_name);
        ...
        SELECT CONCAT('Data transfer sql statement from the old database to the new database for table  ',t_name, ' creation complete');

      END IF;

      SELECT CONCAT('Preparing the data transfer sql statement from the old database to the new database for table  ',t_name);

      PREPARE stmt FROM @q_statment;

      SELECT CONCAT('Executing the data transfer sql statement from the old database to the new database for table  ',t_name);

      EXECUTE stmt;

      SELECT CONCAT('Data transfer sql statement from the old database to the new database for table  ',t_name, ' execution complete');
    ...
    SELECT 'Done doing data exports, please wait updating users and providers';

    -- Removed the transfer of locations only updating the main location with the values from the old database
    SELECT 'Updating the location information in the new database with values in the old database';
    ...
    SELECT 'Exporting providers from old database';
...
BEGIN
  ...
  SELECT 'Trying to merge summary pages if there are patients with more than one';
...
SELECT 'Database export complete';
