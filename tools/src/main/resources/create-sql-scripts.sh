...
cat database/transform_encounters_from_old_to_new.sql > upgrade/upgrade.sql
...
cat database/1.6_to_1.11_data_transfer.sql >> upgrade/upgrade.sql

# add visits to encounters - which is required in 1.11.x
cat database/add_visit_to_encounter.sql >> upgrade/upgrade.sql
