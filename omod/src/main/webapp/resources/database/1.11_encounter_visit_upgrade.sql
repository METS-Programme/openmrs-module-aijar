...
SELECT patient_id, 1, encounter_datetime, encounter_datetime, location_id, creator, encounter_datetime, UUID(), creator, encounter_datetime FROM encounter;

/** Add the visit created above to the encounter for which it belongs **/
UPDATE encounter e INNER JOIN visit v ON (e.patient_id = v.patient_id AND e.encounter_datetime = v.date_started AND e.creator = v.creator AND e.location_id = v.location_id)
